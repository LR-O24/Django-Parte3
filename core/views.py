...
def signup_oficina(request):
    ...
    # Reutiliza o template signup.html, passando o contexto 'Oficina'
...
@require_POST # Decorador NOVO: Garante que só aceita POST
@login_required
def pegar_servico(request, pk):
    problema = get_object_or_404(Problema, pk=pk)
    
    # Verifica se o problema está aberto e se o usuário é uma oficina
    if problema.status == 'ABERTO' and request.user.is_oficina:
        # ATUALIZAÇÃO: Verifica se já não foi pego por outra oficina no meio tempo
        if not problema.oficina: 
            problema.oficina = request.user
            problema.status = 'ANDAMENTO'
            problema.save()
            # Opcional: Adicionar uma mensagem de sucesso
            # messages.success(request, f'Serviço "{problema.titulo}" pego com sucesso!')
        # else:
            # Opcional: Adicionar mensagem de erro se o serviço foi pego recentemente
            # messages.error(request, 'Este serviço já foi pego por outra oficina.')
...
def concluir_servico(request, pk):
    problema = get_object_or_404(Problema, pk=pk)
    # Garante que apenas a oficina responsável e com status ANDAMENTO pode concluir
    if problema.oficina == request.user and problema.status == 'ANDAMENTO':
        problema.status = 'CONCLUIDO'
        problema.save()
    return redirect('dashboard_oficina')